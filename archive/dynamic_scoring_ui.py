...
def calculate_dynamic_score(form_data: Dict, manager: DynamicScorecardManager) -> Dict[str, Any]:
    """Calculate overall score using dynamic configuration"""
    
    variables = manager.get_active_variables()
    
    if not variables:
        st.error("No active variables found for scoring")
        return None
    ...
    for variable in variables:
        ...
        weight = variable['weight'] / 100.0  # Convert percentage to decimal
        ...
        if var_id in form_data:
            ...
            if "error" not in score_result:
                variable_score = score_result['score']
                ...
                st.warning(f"Could not score {variable['display_name']}: {score_result.get('error', 'Unknown error')}")
...
def render_dynamic_scoring_results(results: Dict[str, Any], form_data: Dict, manager: DynamicScorecardManager):
    ...
    for category in categories:
        ...
        if category_vars:
            with st.expander(f"{category['icon']} {category['category_name']} ({len(category_vars)} variables)"):
                
                category_total_score = sum(v["weighted_score"] for v in category_vars)
                category_max_score = sum(v["weight"]/100 for v in category_vars)
                category_percentage = (category_total_score / category_max_score * 100) if category_max_score > 0 else 0
                
                st.markdown(f"**Category Score: {category_percentage:.1f}%**")
