...
class DynamicScorecardManager:
    ...
    def update_category(self, category_name: str, new_name: str, new_icon: str, new_weight: float):
        ...
        try:
            cursor.execute('''
                UPDATE scorecard_categories 
                SET category_name = ?, icon = ?, total_weight = ?
                WHERE category_name = ?
            ''', (new_name, new_icon, new_weight, category_name))
            
            # If category name changed, update all variables in this category
    ...
    def update_variable_weight(self, variable_id: str, new_weight: float):
        """Update variable weight and sync to scoring weights file"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        cursor.execute('''
            ...
            WHERE variable_id = ?
        ''', (new_weight, variable_id))
        
        conn.commit()
        conn.close()
        
        # Sync to scoring weights file
        self._sync_weights_to_file()
    
    def _sync_weights_to_file(self):
        """Sync all weights from database to JSON file"""
        try:
            import json
            
            # Get all current weights from database
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            ...
            # Convert to scoring system format
            weights = {}
            for variable_id, weight in results:
                weights[variable_id] = float(weight) / 100.0  # Convert percentage to decimal
            
            # Save directly to scoring_weights.json
    ...
    def sync_weights_from_file(self):
        """Sync weights from JSON file to database"""
        try:
            # Read weights directly from scoring_weights.json
            import json
            with open("scoring_weights.json", "r") as f:
                json_weights = json.load(f)
            
            # Update database with slider values
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            
            for variable_id, weight in json_weights.items():
                weight_percentage = float(weight) * 100.0  # Convert decimal to percentage
                cursor.execute('''
                    UPDATE scorecard_variables 
                    SET weight = ?, updated_at = datetime('now')
                    WHERE variable_id = ?
                ''', (weight_percentage, variable_id))
            ...
            print(f"Error syncing from file: {e}")
            return False
    
    def sync_weights_to_file(self):
        """Public method to sync weights to file"""
        self._sync_weights_to_file()
        return True